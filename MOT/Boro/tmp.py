...
class MOT:
    ...
    def animate_Ei1_2D(self,curve, E_i=None, interval=30):
        x = np.linspace(-2*np.max(curve[0,:]), 2*np.max(curve[0,:]), 300)
        y = np.linspace(-2*np.max(curve[1,:]), 2*np.max(curve[1,:]), 300)
        ...
        # ---------- CASE 2: precomputed ----------
            ...
            def update(frame):
                im.set_array(Ei_array[:, frame].reshape(X.shape))
